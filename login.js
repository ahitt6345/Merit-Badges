...
var getStudents = function() {
    try {
        students = fs.readFileSync('students.txt', 'utf8').split('\n').map(JSON.parse); // convert data to an array of javascript objects
    } catch (e) {
        console.log(e);
        students = [];
    }
...
var createNewStudent = function(name, lastInitial, school) {
    ...
    students.push([name, lastInitial, school, []]);
};
...
io.on('connection', function(socket) {
    ...
    socket.on('getBadges', function() {
        socket.emit('studentBadges');
    });
});